...
let __iconify_utils_lib_loader_node_loader = require("@iconify/utils/lib/loader/node-loader");
__iconify_utils_lib_loader_node_loader = require_chunk.__toESM(__iconify_utils_lib_loader_node_loader);
...
const JSXCompiler = (async (svg, collection, icon, options) => {
	const svgrCore = await (0, local_pkg.importModule)("@svgr/core");
	...
	}, { componentName: (0, __iconify_utils_lib_misc_strings.camelize)(`${collection}-${icon}`) });
...
const QwikCompiler = (async (svg, collection, icon, options) => {
	...
	const toJsxComponent = (await (0, local_pkg.importModule)("@svgx/core")).toJsxComponent;
...
const Vue2Compiler = (async (svg, collection, icon) => {
	const { compile } = await (0, local_pkg.importModule)("vue-template-compiler");
	const transpileMod = await (0, local_pkg.importModule)("vue-template-es2015-compiler");
...
const Vue3Compiler = (async (svg, collection, icon) => {
	const { compileTemplate } = await (0, local_pkg.importModule)("@vue/compiler-sfc");
	const { injectScripts, svg: handled } = handleSVGId(svg);
...
const WebComponentsCompiler = ((svg, collection, icon, { webComponents: options }) => {
	...
	const name = (0, __iconify_utils_lib_misc_strings.camelize)(id);
...
async function generateComponent({ collection, icon, query }, options) {
	...
	const svg = await (0, __iconify_utils_lib_loader_node_loader.loadNodeIcon)(collection, icon, {
		addXmlNs: false,
		scale,
		customCollections,
		autoInstall,
...
const debug$1 = (0, debug.default)("unplugin-icons:options");
async function resolveOptions(options) {
	const { scale = 1.2, defaultStyle = "", defaultClass = "", compiler = await guessCompiler(), jsx = guessJSX(), customCollections = {}, iconCustomizer = () => {}, transform, autoInstall = false, collectionsNodeResolvePath = node_process.default.cwd() } = options;
	const webComponents = Object.assign({
		autoDefine: false,
		iconPrefix: "icon"
	}, options.webComponents);
	debug$1("compiler", compiler);
	return {
		scale,
		defaultStyle,
		defaultClass,
...
async function guessCompiler() {
	return await getVueVersion() || ((0, local_pkg.isPackageExists)("@svgr/core") ? "jsx" : "vue3");
}
function guessJSX() {
	if ((0, local_pkg.isPackageExists)("preact")) return "preact";
	return "react";
}
async function getVueVersion() {
	try {
		const result = await (0, local_pkg.getPackageInfo)("vue");
